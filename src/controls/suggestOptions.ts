/**
 * `suggestOptions` — the ranked substring matcher behind `TypeaheadControl`, generalized from
 * aml-v2's `suggestCountries` and promoted verbatim out of `@dloizides/ui-tables`' private
 * `Filters/utils/suggest.ts`.
 *
 * Ranks: value exact (0) > value prefix (1) > label prefix (2) > label contains (3); ties broken
 * by `label.localeCompare`. Empty/short queries return `[]` — nothing surfaces until `minChars`
 * is typed, mirroring a native `<datalist>`.
 */
...
function rankOf(option: ControlOption, needle: string): number {
...
export function suggestOptions(
  ...
  const scored: Array<{ option: ControlOption; rank: number }> = [];
...
export function isUnmatched(options: readonly ControlOption[], text: string): boolean {
